...
class OfferList extends React.Component {
  ...
  handleStatusChange = (sid) => {
    ...
    let newArray = this.state.offer_list;
    var a = newArray.find((element) => {
      return element.offerID === sid
    })
    a.status = status;
    console.log(newArray)
    this.setState({ offer_list: newArray })
    ...
    const columns = [
      {
        ...
        options: {
          filter: true,
          sort: true,
          customBodyRender: (offerID, tableMeta) => {
      ...
      },
      {
        ...
        options: {
          filter: true,
          sort: true,
          customBodyRender: offerID => {
            return (
              <div>
                <Link to={"/offer/edit/" + hashids.encode(offerID)}
                ...
                <span onClick={this.deletepackage.bind(this, offerID)}
                  className="m-r-15 text-muted"
                  data-toggle="tooltip"
                  data-placement="top"
                  title=""
                          ...
                          <MUIDataTable
                            title={
                              <div className="card-header">
                                <h5>offer List</h5>
                                <Link to="/offer/add"
                                  className="btn btn-sm btn-primary waves-effect waves-light f-right d-inline-block md-trigger" data-modal="modal-13"> <i className="icofont icofont-plus m-r-5"></i> Add offer
                             </Link>
                              </div>
                            }
                            data={this.state.offer_list}
                            columns={columns}
                            options={options}
                          />
