...
class LanguageList extends React.Component {
  ...
  handleStatusChange = (sid) => {
    ...
    let newArray = this.state.language_data;
    var a = newArray.find((element) => {
      return element.language_id === sid
    })
    a.status = status;
    console.log(newArray)
    this.setState({ language_data: newArray })
    ...
    const columns = [
      {
        ...
        options: {
          filter: true,
          sort: true,
          customBodyRender: (language_id, tableMeta) => {
      ...
      },
      {
        ...
        options: {
          filter: true,
          sort: true,
          customBodyRender: language_id => {
            return (
              <div>
                {" "}
                <Link to={"/language/edit/" + hashids.encode(language_id)}
                ...
                <span onClick={this.deletepackage.bind(this, language_id)}
                  className="m-r-15 text-muted"
                  data-toggle="tooltip"
                  data-placement="top"
                  title=""
                          ...
                          <MUIDataTable
                            title={
                              ...
                              <Link to="/language/add"
                                className="btn btn-sm btn-primary waves-effect waves-light f-right d-inline-block md-trigger" data-modal="modal-13"> <i className="icofont icofont-plus m-r-5"></i> Add Language
                             </Link>
                            </div>
                            }
